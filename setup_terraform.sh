...
export TF_VAR_number_of_workers=35
...
if ! awk -v ver="$(get_ssh_version)" 'BEGIN { if (ver < 6.9) exit 1; }'; then
    export TF_VAR_ssh_fingerprint=$(ssh-keygen -lf ./secrets/id_rsa.pub | awk '{print $2}')
else
    export TF_VAR_ssh_fingerprint=$(ssh-keygen -E MD5 -lf ./secrets/id_rsa.pub | awk '{print $2}' | sed 's/MD5://g')
fi
